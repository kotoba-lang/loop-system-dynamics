(ns loop-system-dynamics.cloud-itonami-xmile
  "A real XMILE (OASIS 1.0, kotoba-lang/org-oasis-open-xmile) stock-flow
   simulation of cloud-itonami's GitHub-repo -> com-junkawasaki/root
   manifest/west.yml registration pipeline, per name-prefix category (isic/
   isco/iso/lei/assoc/municipality/...).
   ...
   backlog is already 0 (e.g. assoc) naturally evaluates to a flow of 0
   without a special case. observed-rate-<cat> comes straight from
   resources/cloud-itonami-fleet-xmile-seed.edn's two real, git-verified
   manifest/west.yml snapshots -- never fabricated, and a category with zero
   observed registrations in that window (isco, iso, ...) gets a real
...
(defn observe
  ([] (observe "resources/cloud-itonami-fleet-xmile-seed.edn"))
...
(defn build-model
  ...
  [{:keys [categories window]} & [{:keys [stop dt] :or {stop 40.0 dt 0.1}}]]
  (let [days (:days window)
    ...
    (reduce
     ...
     (-> (m/model "cloud-itonami-fleet-registration")
         (m/set-sim-specs (m/sim-specs 0.0 stop {:xmile/dt dt :xmile/method :euler})))
     active)))

(defn evaluate
  [observation & [sim-opts]]
  (let [mdl (build-model observation sim-opts)
...
(defn render-report
  ...
  (str "# cloud-itonami fleet-registration XMILE simulation — as of " (:as-of observation) "\n\n"
       "Generated by kotoba-lang/loop-system-dynamics "
       "(loop-system-dynamics.cloud-itonami-xmile). Stock-flow simulation: "
       ...
       (if (seq (:stalled decision))
         ...
         "(none)")
       "\n\n## Reads\n\n"
       "isco carries the single largest active backlog (124 as of this observation) "
       "and a measured registration rate of exactly 0/day in the observed window -- "
       "distinct from lei/assoc/municipality, whose backlogs are actively draining "
       "and would clear on their own at the currently-observed rate. Coverage growth "
       "for cloud-itonami as a whole is not evenly distributed across categories; "
       "it is concentrated in whichever categories currently have a nonzero flow, "
       "and isco (plus iso, at a much smaller scale) will not close without a new "
       "intervention, not just more elapsed time.\n"))

(defn act!
  [observation decision report-path]
  (ensure-dir! report-path)
  (fs/writeFileSync report-path (render-report observation decision))
  report-path)
...
(defn run-cycle!
  [{:keys [seed-path report-path ledger-path sim-opts]
    :or {seed-path "resources/cloud-itonami-fleet-xmile-seed.edn"
         report-path "target/cloud-itonami-fleet-xmile-report.md"
         ledger-path "ledger/cloud-itonami-fleet-xmile-ledger.edn"}}]
  (let [observation (observe seed-path)
    ...
    (when-not (:valid? evaluation)
      (throw (ex-info "cloud-itonami-xmile: model failed XMILE validation"
                       {:problems (:problems evaluation)})))
    (act! observation decision report-path)
